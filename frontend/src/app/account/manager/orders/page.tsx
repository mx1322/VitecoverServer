...
export default function ManagerOrdersPage() {
  ...
  if (role !== "product_manager" && role !== "admin") {
    return (
      <section className="rounded-[28px] border border-[rgba(22,36,58,0.08)] bg-[rgba(255,255,255,0.94)] p-6">
        <h2 className="text-2xl font-semibold text-[var(--ink)]">{t(locale, "Product managers only")}</h2>
      </section>
    );
  }
    ...
    setOrders((current) =>
      current.map((order) => {
        ...
        if ((order.vehicleVerified || order.driverVerified) && patch.reviewStatus !== undefined) {
          setMessage(t(locale, "The review state is locked for orders with verified documents. An administrator must change it."));
          return order;
        }
        ...
        if (next.reviewStatus === "pending_confirmation" && next.paymentStatus !== "paid") {
          setMessage(t(locale, "All orders waiting for confirmation must already be paid. This change was blocked."));
          return order;
        }
  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[28px] border border-[rgba(22,36,58,0.08)] bg-[rgba(255,255,255,0.94)] p-6">
        <h2 className="text-3xl font-semibold tracking-tight text-[var(--ink)]">{t(locale, "Product manager order review")}</h2>
        <p className="mt-3 text-sm text-[var(--muted)]">
          {t(locale, "Rules: orders waiting for confirmation must already be paid. Managers can cancel or edit orders. If either the driver or vehicle documents have been verified, the order is locked and can only be changed by an administrator.")}
        </p>
      </section>
        ...
        return (
          <article key={order.id} className="rounded-[22px] border border-[rgba(22,36,58,0.08)] bg-white px-5 py-5">
            <div className="grid gap-3 md:grid-cols-2">
              ...
              <label className="text-sm">
                ...
                <select
                  ...
                  className="mt-2 w-full rounded-xl border px-3 py-2 text-sm disabled:bg-[rgba(235,235,235,0.6)]"
                >
                  <option value="pending_confirmation">{t(locale, "pending_confirmation")}</option>
            ...
            <div className="mt-4 flex flex-wrap gap-3">
              ...
              >
                {t(locale, "Cancel order")}
              </button>
              <span className="text-xs text-[var(--muted)]">
                {t(locale, "Document lock")}: {locked ? t(locale, "Locked (administrator required)") : t(locale, "Unlocked")}
              </span>
            </div>
          </article>
        );

  ...
  return (
    <div className="space-y-6">
      <section className="rounded-[28px] border border-[rgba(22,36,58,0.08)] bg-[rgba(255,255,255,0.94)] p-6 shadow-[0_18px_50px_rgba(22,36,58,0.05)]">
        ...
        <h2 className="mt-3 text-3xl font-semibold tracking-tight text-[var(--ink)]">{t(locale, "Documents")}</h2>
        <p className="mt-3 text-sm leading-6 text-[var(--muted)]">
          {t(locale, "Track verification status and upload any requested files.")}
        </p>
      </section>
        ...
        {documents.map((document) => (
          ...
          >
            <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
              <div>
                <p className="font-semibold text-[var(--ink)]">{t(locale, document.name)}</p>
                <p className="mt-1 text-sm text-[var(--muted)]">{t(locale, document.note)}</p>
              </div>
              <span className="rounded-full bg-[rgba(234,237,241,1)] px-3 py-1.5 text-xs font-semibold text-[var(--ink)]">
                {t(locale, document.status)}
              </span>
            </div>
          </article>
        ))}

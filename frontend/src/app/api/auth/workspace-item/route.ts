...
async function getEmailFromSession(): Promise<string> {
  ...
  if (!account) {
    throw new Error("You must be signed in to manage account records.");
  }
...
export async function POST(request: Request) {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Unable to create this account record.";
...
export async function DELETE(request: Request) {
  try {
    ...
    const message =
      error instanceof Error ? error.message : "Unable to delete this account record.";

...
export async function generateMetadata({ params }: { params: Promise<{ locale: string }> }): Promise<Metadata> {
  ...
  const home = getHomeContent(locale as Locale);
...
export default async function LocaleHomePage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [dictionary, home] = await Promise.all([getDictionary(locale), Promise.resolve(getHomeContent(locale as Locale))]);
    ...
    <main className="mx-auto max-w-6xl px-6 py-12">
      <section className="rounded-[32px] border border-[rgba(22,36,58,0.08)] bg-[rgba(255,255,255,0.94)] p-8 shadow-[0_18px_50px_rgba(22,36,58,0.05)]">
        ...
        <div className="mt-6 flex gap-3">
          <Link href={`/${locale}/quote`} className="rounded-full bg-[var(--accent)] px-5 py-3 text-sm font-semibold text-[var(--ink)]">{home.hero.primaryCtaLabel ?? dictionary.cta.getQuote}</Link>
          <Link href={`/${locale}/products`} className="rounded-full border px-5 py-3 text-sm font-semibold text-[var(--ink)]">{home.hero.secondaryCtaLabel ?? dictionary.nav.products}</Link>
        </div>
      </section>

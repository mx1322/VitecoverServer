...
import { getProductsContent } from "@/lib/content/get-content";
...
export default async function ProductsPage({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [dictionary, products] = await Promise.all([getDictionary(locale), getProducts()]);
  const localizedProducts = getProductsContent(locale as Locale);
      ...
      {products.length === 0 ? <p className="mt-8 rounded-2xl border p-6 text-sm text-[var(--muted)]">{dictionary.products.noData}</p> : (
        <div className="mt-8 grid gap-4 md:grid-cols-2 xl:grid-cols-3">
          {products.map((product) => {
            const content = localizedProducts[product.code] ?? localizedProducts[product.slug];
            return (
              <article key={product.id} className="rounded-2xl border bg-white/90 p-5">
                <h2 className="text-xl font-semibold">{content?.title ?? product.code}</h2>
                <p className="mt-2 text-sm text-[var(--muted)]">{content?.shortDescription ?? dictionary.products.noData}</p>

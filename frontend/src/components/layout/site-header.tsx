    ...
    <header className="sticky top-0 z-20 border-b border-[rgba(22,36,58,0.08)] bg-[rgba(255,252,247,0.94)] backdrop-blur">
      <div className="mx-auto flex max-w-6xl items-center justify-between gap-6 px-6 py-4 md:py-5">
        <Link href={`/${locale}`} className="flex items-center gap-4 text-[var(--ink)]">
          <span className="inline-flex h-[3.25rem] w-[3.25rem] items-center justify-center rounded-full bg-[var(--accent)] text-base font-semibold">{dictionary.brandInitials}</span>
          <span>
            <span className="block text-xl font-semibold tracking-tight md:text-[1.75rem]">{dictionary.brandName}</span>
        ...
        <div className="flex items-center gap-3">
          <Suspense fallback={null}><LanguageSwitcher label={dictionary.common.languageSwitcherLabel} /></Suspense>

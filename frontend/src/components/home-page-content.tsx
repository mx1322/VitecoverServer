...
import { homeCopy, localePath, localeToFaqLocale, type Locale } from "@/lib/i18n";
    ...
    <main>
      ...
      <section id="products" className="section-wrap pb-16 md:pb-24">
        <div className="max-w-3xl">
          <p className="mt-4 text-lg leading-7 text-[var(--muted)]">{copy.productsIntro}</p>
        </div>
        ...
        <div className="mt-5 grid gap-8 md:grid-cols-2 xl:grid-cols-3">
          {homeProductCards.map((product) => {
            ...
            const cardBody = (
              <article className={cardShellClass}>
                ...
                <div className={priceBlockClass}>
                  ...
                  <p className="mt-2 whitespace-nowrap text-[28px] font-semibold leading-none tracking-tight text-[var(--ink)]">
                    {priceMain}
                    {priceSuffix ? (
            ...
            return product.available && product.href ? (
              <Link key={product.code} href={localePath(locale, product.href)} className="block">
                {cardBody}
              </Link>
            ) : (
      ...
      <section className="section-wrap pb-16 md:pb-24">
        <div className="rounded-[36px] border border-[rgba(22,36,58,0.08)] bg-[rgba(255,255,255,0.88)] p-8 shadow-[0_20px_60px_rgba(22,36,58,0.06)] md:p-10">
          <div className="max-w-2xl">
            ...
            <p className="mt-4 text-base leading-7 text-[var(--muted)]">{copy.howItWorksIntro}</p>
          </div>
      ...
      <section className="section-wrap pb-16 md:pb-24">
        <FaqLinkPanel
          title={copy.faqTitle}
          intro={copy.faqIntro}
          items={getFaqByTag("home", localeToFaqLocale(locale)).slice(0, 3)}
          locale={locale}
        />
      </section>
